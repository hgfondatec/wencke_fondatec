WITH t1 AS (
    ...
    SELECT *
    FROM {{ ref('glasofix_prep_rechnung_belege') }}
...
rechnung_rezept_ref AS (
    ...
    FROM {{ ref('glasofix_bronze_positionen') }} pos
...
pauschalen AS (
    ...
    SELECT *
    FROM {{ ref('glasofix_bronze_positionen') }}
...
LEFT JOIN {{ ref('glasofix_prep_beleggruppe') }} beleggruppe 
