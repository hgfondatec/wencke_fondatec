...
with positionen AS (
    SELECT *
    FROM {{ ref('glasofix_silver_rekla_positionen') }}
),

belege AS (
    SELECT *
    FROM {{ ref('glasofix_prep_rekla_belege') }}
),

rekla_grund AS (
    SELECT *
    FROM {{ ref('glasofix_bronze_rekla_grund') }}
),
...
rekla_massnahme AS (
    ...
    FROM {{ ref('glasofix_bronze_rekla_massnahme') }}
)
