...
WITH belege_positionen AS (
    ...
    FROM {{ ref('silver_wencke_belege') }} b
...
SELECT
    *,
    ...
    CASE
        WHEN bg_beleggruppe IN ('G00', 'G01', 'G02')
            THEN -1
        WHEN bg_beleggruppe IN ('R00', 'R83')
            THEN 1
        ELSE 0
    ...
    CASE
        WHEN bel_steuer_art IN ('3', '5')
             AND bg_beleggruppe IN ('R00', 'R83')
            THEN COALESCE(pos_gesamtbetrag, 0)

        WHEN bel_steuer_art IN ('3', '5')
             AND bg_beleggruppe IN ('G00', 'G01', 'G02')
    ...
    COALESCE(pos_ek_betrag, 0)
        * COALESCE(pos_menge, 0)
        * CASE
            WHEN bg_beleggruppe IN ('G00', 'G01', 'G02') THEN -1
            ELSE 1
