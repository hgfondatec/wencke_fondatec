...
WITH silver_belege_positionen AS (
    SELECT *
    FROM {{ ref('lloyd_silver_belege_positionen') }}
)
...
LEFT JOIN {{ ref('lloyd_gold_artikel') }} ar
...
LEFT JOIN {{ ref('lloyd_bronze_lieferadressen') }} lieferadressen
