...
WITH silver_belege_positionen AS (
    SELECT *
    FROM {{ ref('vms_silver_belege_positionen') }}
)
...
LEFT JOIN {{ ref('vms_gold_artikel') }} ar
...
LEFT JOIN {{ ref('vms_bronze_lieferadressen') }} lieferadressen
