...
select 
    ...
    case
        when rechnung.rechnung_steuerart IN('3','5') and _beleggruppe_rechnung.bg_beleggruppe IN ('R00','R83') then _position_rechnung.pos_gesamtumsatz
        when rechnung.rechnung_steuerart IN('3','5') and _beleggruppe_rechnung.bg_beleggruppe IN ('G00','G01','G02') then _position_rechnung.pos_gesamtumsatz*(-1)
        when rechnung.rechnung_steuerart <> '3' and _beleggruppe_rechnung.bg_beleggruppe IN ('R00','R83') then _position_rechnung.pos_gesamtrohertrag 
             + (_position_rechnung.pos_ek_einzeln * _position_rechnung.pos_gesamtmenge)   
        when rechnung.rechnung_steuerart <> '3' and _beleggruppe_rechnung.bg_beleggruppe IN ('G00','G01','G02') then (_position_rechnung.pos_gesamtrohertrag 
...
--left join position _position_auftrag
    --on _position_auftrag.bel_belegnummer = CAST(auftrag.auftrag_beleg_nr as varchar(14))

--left join position _position_lieferung
    --on CAST(lieferung.lieferung_beleg_nr as varchar(14)) = _position_lieferung.bel_belegnummer 
...
left join beleggruppe _beleggruppe_rechnung
    on rechnung.rechnung_beleggruppe = LPAD(CAST(_beleggruppe_rechnung.bg_beleggruppe_id as varchar(14)), 2, '0')
